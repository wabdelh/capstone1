...
class ManageUser : AppCompatActivity(), View.OnClickListener {
                        ...
                        .addListenerForSingleValueEvent(object :
                            ...
                            override fun onDataChange(d: DataSnapshot) {
                                for (snapshot in d.children) {
                                    ...
                                    u =
                                        snapshot.getValue(User::class.java)!! //this puts the value with a given email into the User class
                                }
                                u.email = email //change the email
                                database.child(key).setValue(u)
                                    .addOnSuccessListener { //this is run after the key
                                        Toast.makeText(
                                            this@ManageUser,
        ...
        database.orderByChild("email").equalTo(email).addListenerForSingleValueEvent(object :
            ...
            override fun onDataChange(d: DataSnapshot) {
                ...
                database.child(key).setValue(userData)
                    .addOnSuccessListener { //why does this have to be in the onDataChange? It does
                        Toast.makeText(
                            this@ManageUser,
    ...
    private fun deleteUser() {
        ...
        val userEmailString: String =
            user.email.toString().trim() // get current user's email for later use
            ...
            user.reauthenticate(credential) // user need to be re-authenticated before deletion
                .addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
                        user.delete() // delete in Authentication
                            .addOnCompleteListener { task ->
                                if (task.isSuccessful) {
                                    ...
                                    uDatabase.orderByChild("email").equalTo(user.email as String)
                                        .addListenerForSingleValueEvent(object :
                                            ValueEventListener {
                                            override fun onDataChange(d: DataSnapshot) {
                                                for (snapshot in d.children) {
                                                ...
                                                uDatabase.child(key).removeValue()
                                                    .addOnSuccessListener { // delete in Realtime Database
                                                        Toast.makeText(
                                                            this@ManageUser,
                        ...
                        Toast.makeText(
                            baseContext,
                            "Re-auth failed, please confirm credentials.",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
        ...
        builder.setNegativeButton(
            "Cancel",
            DialogInterface.OnClickListener { builder, which -> builder.cancel() })
        builder.show()
